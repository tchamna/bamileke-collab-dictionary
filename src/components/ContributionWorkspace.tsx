      ...
      <section className="border-b border-[#d9d8cc] bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-medium uppercase tracking-wide text-[#6c6f67]">{t.appName}</p>
              <h1 className="mt-1 text-3xl font-semibold tracking-normal text-[#20231f]">{t.translateFrenchList}</h1>
          ...
          <div className="max-h-[calc(100vh-250px)] overflow-auto p-2">
            {data.rows.map((word) => (
              ...
              >
                ...
                <span className="mt-1 block text-sm text-[#60665b]">{word.nufi.slice(0, 3).join(' / ') || t.noNufiEntry}</span>
          ...
          {selectedWord ? (
            <>
              <div className="grid gap-4 border-b border-[#e3e3da] pb-5 md:grid-cols-2">
                <div>
                  <p className="text-sm font-medium uppercase tracking-wide text-[#687064]">{t.frenchWord}</p>
                  <p className="mt-2 text-4xl font-semibold text-[#20231f]">{selectedWord.french}</p>
                  {selectedWord.english ? <p className="mt-2 text-base text-[#62685d]">{t.english}: {selectedWord.english}</p> : null}
                </div>
                <div>
                  <p className="text-sm font-medium uppercase tracking-wide text-[#687064]">{t.nufiReference}</p>
