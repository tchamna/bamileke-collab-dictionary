  ...
  if (!globalForPg.collabDictionarySchemaReady) {
    globalForPg.collabDictionarySchemaReady = getPool().query(`
      ...
      CREATE TABLE IF NOT EXISTS contribution_review_events (
        ...
        reviewer_points    INTEGER NOT NULL DEFAULT 25,
      ...
      CREATE INDEX IF NOT EXISTS idx_contribution_review_events_reviewer
        ON contribution_review_events (reviewer_email, created_at DESC);

      CREATE INDEX IF NOT EXISTS idx_contribution_review_events_contributor
        ON contribution_review_events (contributor_email, created_at DESC);
...
async function recordReviewEvents(
  client: PoolClient,
  rows: ReviewEventTarget[],
  nextStatus: string,
  reviewerEmail?: string
) {
  const normalizedReviewerEmail = reviewerEmail?.trim().toLowerCase() ?? '';
  const normalizedNextStatus = nextStatus.trim();
  if (!normalizedReviewerEmail || !['approved', 'rejected'].includes(normalizedNextStatus)) return;

  const changedRows = rows.filter((row) => row.status !== normalizedNextStatus);
  ...
  await client.query(
    ...
    SELECT
      ...
      $2,
      25,
      CASE WHEN $2 = 'rejected' THEN -25 ELSE 0 END
...
export async function getContributorStats(email: string) {
  ...
  const result = await getPool().query<{
    ...
    contributor_scores AS (
      SELECT
        ...
        COUNT(*) FILTER (WHERE status <> 'rejected')::int * 50 AS base_points
      FROM latest_contributions
      GROUP BY contributor_email
    ),
    review_adjustments AS (
      SELECT
        contributor_email,
        COALESCE(SUM(contributor_points), 0)::int AS review_points
    ...
    combined_scores AS (
      SELECT
        contributor_email,
        contribution_count,
        base_points + COALESCE(review_adjustments.review_points, 0) AS points
      FROM contributor_scores
      LEFT JOIN review_adjustments USING (contributor_email)
      WHERE contribution_count > 0 OR COALESCE(review_adjustments.review_points, 0) <> 0
    ),
    ranked_scores AS (
      SELECT
        contributor_email,
        contribution_count,
        points,
        DENSE_RANK() OVER (ORDER BY points DESC, contribution_count DESC) AS rank,
        COUNT(*) OVER () AS ranked_contributor_count
      FROM combined_scores
    )
    SELECT
      COALESCE(contribution_count, 0)::text AS contribution_count,
      COALESCE(points, 0)::text AS points,
      rank::text AS rank,
      COALESCE(ranked_contributor_count, (SELECT COUNT(*) FROM combined_scores), 0)::text AS ranked_contributor_count
    FROM ranked_scores
  ...
  const totalResult = row
    ...
    : await getPool().query<{ total: string }>(`
        ...
        review_adjustments AS (
          SELECT contributor_email, COALESCE(SUM(contributor_points), 0)::int AS review_points
          FROM contribution_review_events
          WHERE contributor_email <> ''
          GROUP BY contributor_email
        )
...
export type ContributorLeaderboardRow = {
  ...
  review_adjustment_points: number;
  points: number;
  language_count: number;
  languages: string[];
...
export type ReviewerLeaderboardRow = {
  ...
  points: number;
  latest_reviewed_at: Date;
  rank: number;
  ranked_reviewer_count: number;
};
...
export async function listContributorLeaderboard(limit = 100) {
  ...
  const result = await getPool().query<ContributorLeaderboardRow>(
    ...
    contributor_scores AS (
      SELECT
        latest_contributions.contributor_email,
        COALESCE(latest_names.contributor_name, '') AS contributor_name,
        COUNT(*) FILTER (WHERE latest_contributions.status <> 'rejected')::int AS contribution_count,
        COUNT(*) FILTER (WHERE latest_contributions.status = 'approved')::int AS approved_count,
        COUNT(*) FILTER (WHERE latest_contributions.status = 'pending')::int AS pending_count,
        COUNT(DISTINCT latest_contributions.language) FILTER (WHERE latest_contributions.status <> 'rejected')::int AS language_count,
        COALESCE(
          ARRAY_AGG(DISTINCT latest_contributions.language ORDER BY latest_contributions.language)
            FILTER (WHERE latest_contributions.status <> 'rejected'),
          ARRAY[]::text[]
        ) AS languages,
        MAX(latest_contributions.created_at) AS latest_contribution_at
      FROM latest_contributions
      LEFT JOIN latest_names ON latest_names.contributor_email = latest_contributions.contributor_email
      GROUP BY latest_contributions.contributor_email, latest_names.contributor_name
    ),
    review_adjustments AS (
      SELECT
        contributor_email,
        COALESCE(SUM(contributor_points), 0)::int AS review_adjustment_points
      FROM contribution_review_events
      WHERE contributor_email <> ''
      GROUP BY contributor_email
    ),
    combined_scores AS (
      SELECT
        contributor_scores.*,
        COALESCE(review_adjustments.review_adjustment_points, 0) AS review_adjustment_points,
        contribution_count * 50 + COALESCE(review_adjustments.review_adjustment_points, 0) AS points
      FROM contributor_scores
      LEFT JOIN review_adjustments USING (contributor_email)
      WHERE contribution_count > 0 OR COALESCE(review_adjustments.review_adjustment_points, 0) <> 0
    ),
    ranked_scores AS (
      SELECT
        *,
...
export async function listReviewerLeaderboard(limit = 100) {
  await ensureSchema();

  const result = await getPool().query<ReviewerLeaderboardRow>(
    `
    WITH reviewer_scores AS (
      SELECT
        reviewer_email,
        COUNT(*)::int AS review_count,
        COUNT(*) FILTER (WHERE next_status = 'approved')::int AS approved_review_count,
        COUNT(*) FILTER (WHERE next_status = 'rejected')::int AS rejected_review_count,
        COALESCE(SUM(reviewer_points), 0)::int AS points,
    ...
    ranked_scores AS (
      SELECT
        *,
        DENSE_RANK() OVER (ORDER BY points DESC, review_count DESC) AS rank,
        COUNT(*) OVER () AS ranked_reviewer_count
    ...
    SELECT
      ...
      latest_reviewed_at,
      rank::int,
      ranked_reviewer_count::int
    FROM ranked_scores
    ORDER BY rank ASC, latest_reviewed_at DESC, reviewer_email ASC
    LIMIT $1
  `,
    [Math.max(1, Math.min(limit, 250))]
  );
...
export async function updateAdminContribution(input: {
  ...
  if (input.status.trim() === 'rejected') {
    ...
    try {
      ...
      `
      SELECT id, word_id, language, translation, synonyms, contributor_email, status
      FROM contributions
      WHERE id = $1
      LIMIT 1
    `,
      ...
      const rowsToReject = await client.query<ReviewEventTarget>(
        `
        SELECT id, contributor_email, status
        FROM contributions
        WHERE word_id = $1
          AND language = $2
          AND translation = $3
      ...
      await recordReviewEvents(client, rowsToReject.rows, 'rejected', input.reviewerEmail);
  ...
  try {
    await client.query('BEGIN');
    const target = await client.query<ReviewEventTarget>(
      `
      SELECT id, contributor_email, status
      FROM contributions
      WHERE id = $1
    ...
    await recordReviewEvents(client, target.rows, input.status, input.reviewerEmail);
...
export async function approveAdminContributions(ids: number[], reviewerEmail?: string) {
  await ensureSchema();
  ...
  try {
    await client.query('BEGIN');
    const targets = await client.query<ReviewEventTarget>(
      `
      SELECT id, contributor_email, status
      FROM contributions
      WHERE id = ANY($1::int[])
    ...
    await recordReviewEvents(client, targets.rows, 'approved', reviewerEmail);
...
export async function rejectAdminContributions(ids: number[], reviewerEmail?: string) {
  await ensureSchema();
  const uniqueIds = [...new Set(ids.filter((id) => Number.isInteger(id) && id > 0))];
  if (uniqueIds.length === 0) return 0;
  ...
  try {
    ...
    const rowsToReject = await client.query<ReviewEventTarget>(
      ...
      SELECT c.id, c.contributor_email, c.status
      FROM contributions c
      INNER JOIN targets t ON c.word_id = t.word_id
    ...
    await recordReviewEvents(client, rowsToReject.rows, 'rejected', reviewerEmail);
